...
api.get("/movies-api/movies", async (ctx, _next) => {
...
api.get("/movies-api/movies/:id", async (ctx, _next) => {
...
api.delete("/movies-api/movies/:id", async (ctx, _next) => {
  const id = parseInt(ctx.params.id, 10);
...
api.post("/movies-api/movies", async (ctx, _next) => {
...
api.put("/movies-api/movies", async (ctx, _next) => {
  const movie = ctx.request.body;
